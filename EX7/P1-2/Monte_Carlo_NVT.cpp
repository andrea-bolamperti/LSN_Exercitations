...
{ 
  ...
  Equilibrate(neq);	//Initial equilibration
  int nconf = 1;
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      Move();
      Measure();
      Instant(); //instantaneous observables
...
//g(r)
    for(int i=igofr; i<igofr+nbins; i++) {	//cycle from 2 to 2+nbins
	double r = (double)(i-igofr) * bin_size;
	double dV = 4.*pi/3. * ( pow(r+bin_size, 3) - pow(r, 3) ); //volume spherical shell
		
	stima_gofr = (blk_av[i]/blk_norm) / (rho * dV * (double)npart);
	glob_av[i] += stima_gofr;
    	glob_av2[i] += stima_gofr*stima_gofr;
    	err_g=Error(glob_av[i],glob_av2[i],iblk);
	Gofr << "    " << iblk <<  "    " << stima_gofr << endl; //instant
	if (iblk==nblk) {
		Gave << "    " << r <<  "    " << glob_av[i]/(double)iblk << "    " << err_g << endl;
	}
	Gofr << endl;
     }
    cout << "----------------------------" << endl << endl;
...
void Equilibrate(int nst) {
	cout << "Equilbrating the system with "<<nst<< " points..." << endl;
	for(int i=0; i<nst; i++)
		Move();
	cout << "Equilibration done and system ready " << endl << endl;
}

void Instant() { 	//instant values of observables
	ofstream Epot, Pres;
	Epot.open("Risultati/epot.inst",ios::app);
    	Pres.open("Risultati/pres.inst",ios::app);
	
	Epot << walker[iv]/(double)npart + vtail << endl; //instant energy per particle
	Pres << rho*temp*(walker[iw]+ptail*(double)npart) / vol << endl; //instant pression
